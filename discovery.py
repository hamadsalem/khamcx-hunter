"""
KHAMCX Hunter Bot - Discovery Engine V4 Multi-Source
====================================================
Paper-mode signal engine for Solana meme tokens.

What changed in V4:
- Dexscreener is no longer treated as the only discovery source.
- Optional GMGN public ranking discovery was added with safe fallback.
- Dexscreener remains the validation layer for pairs, liquidity, volume and price action.
- Two different market modes are supported:
  1) EARLY_PUMPFUN / EARLY_LIQUID
  2) POST_GRADUATION
- Pump.fun tokens are no longer killed only because liquidity is 0.
- WATCH alerts are supported for near-miss tokens.
- Runtime is capped safely so GitHub Actions 'timeout 90' does not kill the bot before saving files.

Important:
This file does not buy or sell. It only discovers, filters, scores, logs, and sends Telegram alerts.
"""
...
# GitHub workflow currently uses: timeout 90 python discovery.py || true
# If the script runs 120s, Linux timeout kills it before final save/summary.
# This keeps the script below 90s unless you later increase the workflow timeout.
...
POLL_INTERVAL_SEC = getattr(config, "POLL_INTERVAL_SEC", 8)
MAX_TOKENS_PER_RUN = getattr(config, "MAX_TOKENS_PER_RUN", 150)

WATCH_SCORE_POST = getattr(config, "WATCH_SCORE_POST", 80)
WATCH_SCORE_EARLY = getattr(config, "WATCH_SCORE_EARLY", 78)
ACCEPT_SCORE_POST = getattr(config, "MIN_SCORE_TO_ACCEPT", 85)
ACCEPT_SCORE_EARLY = getattr(config, "MIN_SCORE_TO_ACCEPT_EARLY", 88)
...
VERBOSE_REJECTS = getattr(config, "VERBOSE_REJECTS", False)
...
def normalize_token_address(item):
    """Extract a Solana token address from multiple API response shapes."""
...
def extract_metrics(pair):
    ...
    if created_at_ms:
        ...
        # GMGN fallback shape sometimes has open_timestamp/pool_creation_timestamp in seconds.
...
def check_kill_switches(metrics):
    ...
    # POST / EARLY_LIQUID protection
    k = getattr(config, "KILL_SWITCHES", {})
    max_pc5 = k.get("max_price_change_5m_kill", 50.0)
    max_pc24 = k.get("max_price_change_24h", 250.0)
    min_liq = k.get("min_liquidity_kill", 6000)
...
def apply_filters(metrics):
    ...
    # POST_GRADUATION - original KHAMCX profile
    f = getattr(config, "HUNTER_FILTERS", {})
    if age < f.get("min_age_seconds", 1800):
        return False, "Too young (<30 min)"
    if age > f.get("max_age_seconds", 28800):
        return False, "Too old (>8h)"
    if mcap < f.get("min_mcap_usd", 25_000):
        return False, "MCap too low"
    if mcap > f.get("max_mcap_usd", 220_000):
        return False, "MCap too high"
    if liq < f.get("min_liquidity_usd", 8_000):
        return False, "Liq too low"
    if liq > f.get("max_liquidity_usd", 80_000):
        return False, "Liq too high"
    if vol5 < f.get("min_volume_5m_usd", 2_000):
        return False, "Vol 5m too low"
    if vol1 < f.get("min_volume_1h_usd", 20_000):
        return False, "Vol 1h too low"
    if buys5 < f.get("min_buys_5m", 25):
        return False, "Buys 5m too few"
    if txns5 < getattr(config, "MIN_TXNS_5M_TOTAL", 30):
        return False, "Txns 5m too few"
    if txns1 < f.get("min_txns_1h", 120):
        return False, "Txns 1h too few"
    if ratio < f.get("min_buy_sell_ratio", 1.2):
        return False, "B/S ratio too low"
    if ratio > f.get("max_buy_sell_ratio", 3.0):
        return False, "B/S ratio suspicious"
    if pc5 < f.get("min_price_change_5m", -12.0):
        return False, "5m drop too big"
    if pc5 > f.get("max_price_change_5m", 20.0):
...
def score_volume_liquidity(metrics):
    ...
    if mode == "EARLY_PUMPFUN":
        # Liquidity is often 0 on pumpfun via Dexscreener, so do not punish it heavily.
        score += 5
...
class DexscreenerClient:
    ...
    async def get_pairs_batch(self, token_addrs):
        ...
        # Dexscreener supports up to around 30 comma-separated token addresses.
...
class GmgnPublicClient:
    """
    Optional public GMGN ranking discovery.
    This is intentionally fail-soft because public web endpoints can change or be blocked.
    If GMGN fails, the bot continues with Dexscreener discovery.
    """
    ...
    async def get_rank(self, period, orderby, direction="desc"):
        ...
        headers = {
            ...
            "User-Agent": "Mozilla/5.0 KHAMCXHunter/4.0",
        }
...
async def collect_candidates(dex_client, gmgn_client):
    ...
    # Convert sets to lists for JSON-friendly internal record.
    records = []
...
async def batch_run():
    ...
    log.info("=" * 60)
    log.info("KHAMCX Hunter Bot V4 Multi-Source - Started")
    log.info("Discovery: Dexscreener + optional GMGN public rank")
    ...
    async with aiohttp.ClientSession(
        timeout=timeout,
        connector=connector,
        headers={"User-Agent": "KHAMCXHunter/4.0"},
    ) as session:
