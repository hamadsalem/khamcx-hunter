"""
KHAMCX Hunter Bot - Discovery Engine Stable V5
===============================================
نسخة مستقرة ومُجربة:
- Dexscreener + GMGN (4 calls فقط)
...
def apply_filters(metrics):
    ...
    if txns5 < getattr(config, "MIN_TXNS_5M_TOTAL", 30): return False, "Txns 5m too few"
...
class GmgnPublicClient:
    """GMGN مع 4 calls فقط (الأكثر استقراراً)"""
...
async def batch_run():
    ...
    log.info("=" * 60)
    log.info("KHAMCX Hunter Bot Stable V5 - Started")
    log.info("Discovery: Dexscreener + GMGN (4 calls)")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector, headers={"User-Agent": "KHAMCXHunter/5.0"}) as session:
