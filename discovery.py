"""
KHAMCX Hunter Bot - Discovery Engine V6 (Free Max)
===================================================
V6 Free Version:
- أضفنا مصدر Pump.fun مجاني (يزيد العدد بشكل كبير)
- زدنا GMGN calls إلى 6 (توازن)
- كل شيء مجاني 100%
"""
...
POLL_INTERVAL_SEC = getattr(config, "POLL_INTERVAL_SEC", 12)
MAX_TOKENS_PER_RUN = getattr(config, "MAX_TOKENS_PER_RUN", 200)

WATCH_SCORE_POST = getattr(config, "WATCH_SCORE_POST", 72)
WATCH_SCORE_EARLY = getattr(config, "WATCH_SCORE_EARLY", 70)
ACCEPT_SCORE_POST = getattr(config, "MIN_SCORE_TO_ACCEPT", 76)
ACCEPT_SCORE_EARLY = getattr(config, "MIN_SCORE_TO_ACCEPT_EARLY", 78)
...
PUMPFUN_API_BASE = getattr(config, "PUMPFUN_API_BASE", "https://frontend-api.pump.fun")
...
def format_telegram_message(token_addr, metrics, score_data, decision="ACCEPT"):
    ...
    if adjustment:
        msg += f"  (Base {base_score}, Adj {adjustment:+d})"
    msg += f"\n\n*Contract:*\n`{token_addr}`\n\n"
    msg += "📊 *Score Breakdown:*\n"
    ...
    msg += "🔗 *Links:*\n"
    msg += f"[Dexscreener](https://dexscreener.com/solana/{token_addr}) | [GMGN](https://gmgn.ai/sol/token/{token_addr}) | [Photon](https://photon-sol.tinyastro.io/en/lp/{token_addr})\n\n"
...
# ============================================================
# METRICS + FILTERS + SCORING (نفس السابق مع تحسينات بسيطة)
# ============================================================
...
def check_kill_switches(metrics):
    ...
    if pc5 > k.get("max_price_change_5m_kill", 90.0):
        return True, "Long green candle"
    if pc24 > k.get("max_price_change_24h", 450.0):
        return True, "Already pumped too much"
    if liq < k.get("min_liquidity_kill", 4000):
...
def apply_filters(metrics):
    ...
    if mode == "EARLY_PUMPFUN":
        if age < 90: return False, "Too fresh"
        if age > 1800: return False, "Early pumpfun too old"
        if mcap < 5000 or mcap > 80000: return False, "MCap out of range"
        if vol5 < 3000: return False, "Vol too low"
        if buys5 < 30: return False, "Buys too few"
        if ratio < 1.0 or ratio > 3.0: return False, "B/S out of range"
        return True, "Early pumpfun passed"

    if mode == "EARLY_LIQUID":
        if age < 180: return False, "Too fresh"
        if age > 1800: return False, "Not early liquid"
        if mcap < 8000 or mcap > 150000: return False, "MCap out of range"
        if liq < 5000: return False, "Liq too low"
        if vol5 < 4000: return False, "Vol too low"
        if ratio < 1.05 or ratio > 3.2: return False, "B/S out of range"
        return True, "Early liquid passed"

    # POST_GRADUATION
    if age < f.get("min_age_seconds", 900): return False, "Too young"
    if age > f.get("max_age_seconds", 43200): return False, "Too old"
    if mcap < f.get("min_mcap_usd", 12000): return False, "MCap too low"
    if mcap > f.get("max_mcap_usd", 350000): return False, "MCap too high"
    if liq < f.get("min_liquidity_usd", 5000): return False, "Liq too low"
    if vol5 < f.get("min_volume_5m_usd", 1200): return False, "Vol 5m too low"
    if vol1 < f.get("min_volume_1h_usd", 12000): return False, "Vol 1h too low"
    if buys5 < f.get("min_buys_5m", 15): return False, "Buys too few"
    if txns1 < f.get("min_txns_1h", 80): return False, "Txns too few"
    if ratio < f.get("min_buy_sell_ratio", 1.05): return False, "B/S too low"
    if ratio > f.get("max_buy_sell_ratio", 3.8): return False, "B/S suspicious"
    if pc5 < f.get("min_price_change_5m", -25.0): return False, "Big drop"
    if pc5 > f.get("max_price_change_5m", 40.0): return False, "Big pump"
...
def calculate_score(metrics):
    ...
    # Chart
    if -10 <= pc5 <= 15: breakdown["chart_strength"] += 10
    elif -20 <= pc5 <= 25: breakdown["chart_strength"] += 6
    if vol5 >= 5000: breakdown["chart_strength"] += 5

    # Volume/Liquidity
    if vol5 >= 8000: breakdown["volume_liquidity"] += 8
    elif vol5 >= 3000: breakdown["volume_liquidity"] += 5
...
# ============================================================
# DISCOVERY CLIENTS (Dexscreener + Pump.fun + GMGN)
# ============================================================
...
class PumpfunClient:
    """مصدر Pump.fun مجاني - يجيب التوكنات الجديدة"""
    def __init__(self, session):
        self.session = session

    async def get_new_tokens(self, limit=50):
        url = f"{PUMPFUN_API_BASE}/coins?offset=0&limit={limit}&sort=created_timestamp&order=DESC&includeNsfw=false"
        try:
            async with self.session.get(url, timeout=aiohttp.ClientTimeout(total=12)) as resp:
                if resp.status != 200: return []
                data = await resp.json()
                ...
                for item in data:
                    addr = normalize_token_address(item)
                    if addr:
                        out.append(source_record(addr, "pumpfun_new", item))
                return out
        except Exception as e:
            log.warning(f"pumpfun error: {e}")
            return []

    async def get_sources(self):
        tokens = await self.get_new_tokens(60)
        log.info(f"pumpfun_new: {len(tokens)} tokens")
        return [tokens]


class GmgnPublicClient:
    """GMGN مع 6 calls (زيادة معتدلة)"""
...
async def collect_candidates(dex_client, pump_client, gmgn_client):
    dex_results, pump_results, gmgn_results = await asyncio.gather(
        dex_client.get_sources(),
        pump_client.get_sources(),
        gmgn_client.get_sources(),
    )

    all_lists = []
    for group in dex_results + pump_results + gmgn_results:
...
async def batch_run():
    ...
    log.info("=" * 60)
    log.info("KHAMCX Hunter Bot V6 Free Max - Started")
    log.info("Discovery: Dexscreener + Pump.fun + GMGN (6 calls)")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector, headers={"User-Agent": "KHAMCXHunter/6.0"}) as session:
        ...
        pump_client = PumpfunClient(session)
        gmgn_client = GmgnPublicClient(session)

        while time.time() < deadline and processed < MAX_TOKENS_PER_RUN:
            records = await collect_candidates(dex_client, pump_client, gmgn_client)
